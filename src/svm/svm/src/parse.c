...
/* mapping from (char *) -> (int) */
KHASH_INIT(global_map, char *, int, 1, kh_str_hash_func, kh_str_hash_equal);
...
static svm_instruction_t scan_instruction(struct svm_vm_t *vm, int *nregs, char *line, khash_t(global_map) *map);
/* Parse a function body, or panic. */
static struct svm_function_t *parse_function(struct svm_vm_t *vm, int arity, int count, FILE *input, khash_t(global_map) *map);
static size_t scan_string(struct svm_vm_t *vm, char *string, khash_t(global_map) *map);
...
struct svm_function_t *svm_parse_module(struct svm_vm_t *vm, FILE *input)
{
    ...
    ssize_t len;
    size_t allocated = 0;
    khash_t(global_map) *global_map = kh_init(global_map);
    ...
    if (sscanf(line, ".load module %u %n", &count, &n) == 1 && line[n] == '\0') {
        ...
        struct svm_function_t *fun = parse_function(vm, 0, count, input, global_map);

        svm_vector_reserve(&vm->globals, kh_size(global_map), &vm->allocator);
        for (unsigned i = 0; i < kh_size(global_map); i++) {
            struct svm_value_t *x = svm_vector_emplace_back(&vm->globals, &vm->allocator);
            svm_value_set_void(x);
        }

        char *key;
        int val; SVM_UNUSED(val);
        kh_foreach(global_map, key, val,
            svm_free(&vm->allocator, key, 0);)
        kh_destroy(global_map, global_map);
        return fun;
    } else {
...
static svm_instruction_t scan_instruction(struct svm_vm_t *vm, int *nregs, char *line, khash_t(global_map) *map)
{
    ...
    #define handle_R1GLOBAL(upper, desc)                          \
        size_t xlen;                                              \
        const char *xtok = tok(&rest, &xlen);                     \
        if (!xtok)                                                \
            error_usage(desc);                                    \
        int8_t x = tolong(xtok, xlen);                            \
        int32_t idx = scan_string(vm, (char *)rest, map);         \
        *nregs = SVM_MAX(*nregs, x);                              \
        return svm_instruction_r1lit(SVM_OPCODE_##upper, x, idx);
...
static size_t scan_string(struct svm_vm_t *vm, char *string, khash_t(global_map) *map)
{
    ...
    int n = 0;
    unsigned len;
    if (sscanf(line, " string %u %n", &len, &n) == 1) {
        line += n;
        ...
        tmp[len] = '\0';
        if (line[0] != '\0')
            svm_panic("Invalid string literal: \"%s\"", string);
        
        uint32_t it = kh_get(global_map, map, tmp);
        if (it == kh_end(map)) {
            int x;
            it = kh_put(global_map, map, tmp, &x);
            kh_val(map, it) = kh_size(map) - 1;
        } else {
            svm_free(&vm->allocator, tmp, 0);
        }
...
static struct svm_function_t *parse_function(struct svm_vm_t *vm, int arity, int count, FILE *input, khash_t(global_map) *map)
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (sscanf(line, ".load function %d %d %d %n", &reg, &arity, &count, &n) && !line[n]) {
            struct svm_value_t val;
            svm_value_set_function(&val, parse_function(vm, arity, count, input, map));
            fun->instructions[i] =
            ...
            fun->instructions[i] = scan_instruction(vm, &fun->nregs, line, map);
        }
    }
    if (line)
        svm_free(&vm->allocator, line, allocated);
