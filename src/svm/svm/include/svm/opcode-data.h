...
X(not, Not, NOT, R2, "r%d := not r%d", "negates rY", {
    uint8_t destreg = svm_instruction_x(instr);
    uint8_t argreg = svm_instruction_y(instr);

    svm_value_set_boolean(
            &vm->current->regs[destreg],
            !svm_value_truthy(&vm->current->regs[argreg]));
})

/* Math ops */
X(eq, Eq, EQ, R3, "r%d := r%d = r%d", "Tests for integer equality", {
    uint8_t destreg = svm_instruction_x(instr);
...
X(lt, Lt, LT, R3, "r%d := r%d < r%d", "Tests for integer equality", {
    uint8_t destreg = svm_instruction_x(instr);
    uint8_t arg1reg = svm_instruction_y(instr);
    uint8_t arg2reg = svm_instruction_z(instr);
...
X(gt, Gt, GT, R3, "r%d := r%d > r%d", "Tests for integer equality", {
    uint8_t destreg = svm_instruction_x(instr);
    uint8_t arg1reg = svm_instruction_y(instr);
    uint8_t arg2reg = svm_instruction_z(instr);
...
X(leq, Leq, LEQ, R3, "r%d := r%d <= r%d", "Tests for integer equality", {
    uint8_t destreg = svm_instruction_x(instr);
    uint8_t arg1reg = svm_instruction_y(instr);
    uint8_t arg2reg = svm_instruction_z(instr);
...
X(geq, Geq, GEQ, R3, "r%d := r%d >= r%d", "Tests for integer equality", {
    uint8_t destreg = svm_instruction_x(instr);
    uint8_t arg1reg = svm_instruction_y(instr);
    uint8_t arg2reg = svm_instruction_z(instr);
...
X(eqq, Eqq, EQQ, R3, "r%d := eq? r%d r%d", "Tests for value equality", {
    uint8_t destreg = svm_instruction_x(instr);
    uint8_t arg1reg = svm_instruction_y(instr);
    uint8_t arg2reg = svm_instruction_z(instr);
    ...
    svm_value_set_boolean(
            &vm->current->regs[destreg],
            svm_value_eq(&vm->current->regs[arg1reg], &vm->current->regs[arg2reg]));
})
...
/* Global variables */
X(setglobal, SetGlobal, SETGLOBAL, R1GLOBAL, "_G[%2$d] := r%1$d", "Sets the global LIT to value rX", {
    uint8_t argreg = svm_instruction_x(instr);
    uint16_t index = svm_instruction_yz(instr);

    svm_vector_at(&vm->globals, index) = vm->current->regs[argreg];
...
X(getglobal, GetGlobal, GETGLOBAL, R1GLOBAL, "r%d := _G[%d]", "Gets the global LIT and stores it in rX", {
    uint8_t destreg = svm_instruction_x(instr);
    uint16_t index = svm_instruction_yz(instr);

    vm->current->regs[destreg] = svm_vector_at(&vm->globals, index);
...
X(nullq, Nullq, NULLQ, R2, "r%d := nullq r%d", "nullq <destreg> <srcreg>", {
    uint8_t destreg = svm_instruction_x(instr);
    uint8_t valreg  = svm_instruction_y(instr);

    svm_value_set_boolean(
            &vm->current->regs[destreg],
            vm->current->regs[valreg].tag == SVM_VALUE_TAG_EMPTYLIST);
})
