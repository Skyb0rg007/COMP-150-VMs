...
/* String functions */
extern struct svm_string_t *svm_string_new(
        struct svm_vm_t *vm, const char *str, size_t len)
    SVM_ATTR_NONNULL(1, 2) SVM_ATTR_RETURNS_NONNULL;
extern bool svm_string_equal(
        struct svm_vm_t *vm,
        const struct svm_string_t *s1,
        const struct svm_string_t *s2)
    SVM_ATTR_NONNULL(1, 2);
extern uint32_t svm_string_hash(
        struct svm_vm_t *vm,
