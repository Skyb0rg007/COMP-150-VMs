...
-- import           Data.Singletons.Prelude.Foldable (And)
-- import           GHC.TypeLits
...
-- type Member (a :: k) (l :: [k]) =
    -- 0 <= IndexOfWithErr a l
        -- (Text "`" :<>: ShowType a :<>: Text "' is not a member of " :<>: ShowType l)
    -- :: Constraint

-- -- | Removes the element from the type-level list
-- type family Remove (a :: k) (l :: [k]) :: [k] where
    -- Remove a '[] = '[]
    -- Remove a (a : as) = Remove a as
    -- Remove a (b : as) = b : Remove a as

-- -- | Returns the index of the type-level list
...
-- -- | Returns the first index that the type exists at in the type-level list
-- -- type IndexOf (a :: k) (l :: [k]) = IndexOf' a l l :: Nat
-- type IndexOf (a :: k) (l :: [k]) =
    -- IndexOfWithErr a l ( ShowType a
                    -- :<>: Text " not found in list: "
                    -- :$$: Text " "
                    -- :<>: ShowType l
                       -- )

-- -- | Returns the index of a in list l, erroring out with err if not found.
-- type family IndexOfWithErr (a :: k) (l :: [k]) (err :: ErrorMessage) :: Nat where
    -- IndexOfWithErr x '[] err = TypeError err
    -- IndexOfWithErr x (x : xs) _ = 0
    -- IndexOfWithErr x (y : xs) err = 1 + IndexOfWithErr x xs err
